...
const TeachersListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/teachers/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('teachers/view')
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/teachers/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	<CanView pagePath="teachers/delete">
		<MultiDelete />
	</CanView>
			</div>
		);
	...
	return (
<main id="TeachersListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Teachers"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath="">
                        <Link to={`/teachers/add`}>
                            <Button label="Add New Teacher" icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
TeachersListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'teachers',
	apiPath: 'teachers/index',
	routeName: 'teacherslist',
