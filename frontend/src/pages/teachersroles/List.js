...
const TeachersrolesListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	const filterSchema = {
		search: {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/teachersroles/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('teachersroles/view')
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/teachersroles/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	<CanView pagePath="teachersroles/delete">
		<MultiDelete />
	</CanView>
			</div>
		);
	...
	return (
<main id="TeachersrolesListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Teachers Roles"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath="">
                        <Link to={`/teachersroles/add`}>
                            <Button label="Add New Teachers Role" icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
TeachersrolesListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'teachersroles',
	apiPath: 'teachersroles/index',
	routeName: 'teachersroleslist',
...
export default TeachersrolesListPage;
