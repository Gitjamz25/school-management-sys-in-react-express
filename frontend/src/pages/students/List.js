...
const StudentsListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/students/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('students/view')
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/students/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('students/edit')
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('students/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	<CanView pagePath="students/delete">
		<MultiDelete />
	</CanView>
			</div>
		);
	...
	return (
<main id="StudentsListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Students"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath="">
                        <Link to={`/students/add`}>
                            <Button label="Add New Student" icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
StudentsListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'students',
	apiPath: 'students/index',
