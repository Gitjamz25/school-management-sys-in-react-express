...
const Form4marksListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	const filterSchema = {
		search: {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/form4marks/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('form4marks/view')
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/form4marks/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/form4marks/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	...
	<CanView pagePath="form4marks/delete">
		<MultiDelete />
	</CanView>
			</div>
		);
	...
	return (
<main id="Form4marksListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Form 4 Marks"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath="">
                        <Link to={`/form4marks/add`}>
                            <Button label="Add New Form 4 Marks" icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
Form4marksListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'form4marks',
	apiPath: 'form4marks/index',
	routeName: 'form4markslist',
...
export default Form4marksListPage;
