...
const AssignmentsListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	const filterSchema = {
		search: {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/assignments/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('assignments/view')
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/assignments/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	<CanView pagePath="assignments/delete">
		<MultiDelete />
	</CanView>
			</div>
		);
	...
	return (
<main id="AssignmentsListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Assignments"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath="">
                        <Link to={`/assignments/add`}>
                            <Button label="Add New Assignment" icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="asignment_name" header="Asignment Name"   ></Column>
...
AssignmentsListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'assignments',
	apiPath: 'assignments/index',
	routeName: 'assignmentslist',
