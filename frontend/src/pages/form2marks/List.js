...
const Form2marksListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	const filterSchema = {
		search: {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/form2marks/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('form2marks/view')
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/form2marks/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/form2marks/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	...
	<CanView pagePath="form2marks/delete">
		<MultiDelete />
	</CanView>
			</div>
		);
	...
	return (
<main id="Form2marksListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Form 2 Marks"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath="">
                        <Link to={`/form2marks/add`}>
                            <Button label="Add New Form 2 Marks" icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
Form2marksListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'form2marks',
	apiPath: 'form2marks/index',
	routeName: 'form2markslist',
...
export default Form2marksListPage;
