...
@Classes_blueprint.route('/index/<fieldname>/<fieldvalue>')
def list(fieldname=None, fieldvalue=None):
    try:
        ...
        if search:
            query = query.filter(
                Classes.class_name.like(f'%{search}%') | 
                Classes.class_teacher.like(f'%{search}%') | 
                Classes.id.like(f'%{search}%') 
            )
        ...
        if orderby:
            ...
            query = query.order_by(text(f'classes.id {ordertype}'))
...
@Classes_blueprint.route('/view/<rec_id>')
def view(rec_id=None):
    try:
        query = Classes.query
        query = query.filter(Classes.id == rec_id)
        query = query.with_entities(
            Classes.id,
            Classes.class_name,
            Classes.class_teacher
...
# Save form data to the Classes table
@Classes_blueprint.route('/add', methods=['POST'])
def add():
    try:
        modeldata = request.body
        form = ClassesAddForm(modeldata)
        errors = [] # list of validation errors
        
        # validate classes form data
        ...
        # check if class_teacher record already exist in the database
        rec_value = str(modeldata['class_teacher'])
        rec_exist = utils.is_unique(Classes, "class_teacher", rec_value)
        if rec_exist:
            return BadRequest(rec_value + " Already exist!")
        
        # save classes records
...
@Classes_blueprint.route('/edit/<rec_id>', methods=['GET', 'POST'])
def edit(rec_id=None):
    try:
        ...
        if request.method == 'POST':
            errors = []
            modeldata = request.body
            form = ClassesEditForm(modeldata, obj=record)
            
            if not form.validate():
                errors.append(form.errors)
            # check if class_teacher already exist in the database
            rec_value = str(modeldata['class_teacher'])
            rec_exist = utils.is_unique(Classes, "class_teacher", rec_value, "id", rec_id)
            if rec_exist:
                return BadRequest(rec_value  + " Already exist!")
...
@Classes_blueprint.route('/delete/<rec_id>')
def delete(rec_id):
    query = Classes.query
    arr_id = rec_id.split(',')
    try:
        query = query.filter(Classes.id.in_(arr_id))
