...
/**
 * Route to list form2marks records
 * @GET /form2marks/index/{fieldname}/{fieldvalue}
 */
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(search){
			let searchFields = DB.Form2Marks.searchFields();
		...
		query.attributes = DB.Form2Marks.listFields();
		let page = parseInt(req.query.page) || 1;
		let limit = parseInt(req.query.limit) || 10;
		let result = await DB.Form2Marks.paginate(query, page, limit);
...
/**
 * Route to view Form2Marks record
 * @GET /form2marks/view/{recid}
 */
router.get('/view/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = DB.Form2Marks.viewFields();
		let record = await DB.Form2Marks.findOne(query);
		if(!record){
			return res.notFound();
		}
...
/**
 * Route to insert Form2Marks record
 * @POST /form2marks/add
 */
router.post('/add/', 
	[
...
, async function (req, res) {
	try{
		...
		//save Form2Marks record
		let record = await DB.Form2Marks.create(modeldata);
...
/**
 * Route to get  Form2Marks record for edit
 * @GET /form2marks/edit/{recid}
 */
router.get('/edit/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = DB.Form2Marks.editFields();
		let record = await DB.Form2Marks.findOne(query);
		if(!record){
			return res.notFound();
		}
...
/**
 * Route to update  Form2Marks record
 * @POST /form2marks/edit/{recid}
 */
...
, async (req, res) => {
	try{
		...
		await DB.Form2Marks.update(modeldata, {where: where});
		record = await DB.Form2Marks.findOne(query);//for audit trail
		const newValues = JSON.stringify(record); 
...
/**
 * Route to delete Form2Marks record by table primary key
 * Multi delete supported by recid separated by comma(,)
 * @GET /form2marks/delete/{recid}
 */
router.get('/delete/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		let records = await DB.Form2Marks.findAll(query);
		records.forEach(async (record) => { 
			//perform action on each record before delete
			const oldValues = JSON.stringify(record); //for audit trail
			req.writeToAuditLog({ recid: record['id'], oldValues });
		});
		await DB.Form2Marks.destroy(query);
		return res.ok(recid);
	}
	catch(err){
		return res.serverError(err);
